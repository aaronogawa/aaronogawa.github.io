...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var worksHoverBarVisible = false;
    function updateWorksHoverBar(rect, hoveredBtn) {
        var isNew = !worksHoverBar;
        if (isNew) {
        ...
        } else if (slug === 'events' || slug === 'misc' || isBackBtn) {
        ...
        if (!worksHoverBarVisible) {
            worksHoverBar.offsetHeight; // force reflow so opacity transition fires
            worksHoverBarVisible = true;
        }
    ...
    if (worksMainNav) {
        setTimeout(function() { document.documentElement.classList.remove('works-nav-animate'); }, 700);
    }

    if (document.documentElement.classList.contains('scroll-enter')) {
        var scrollEnterIndicator = document.querySelector('.scroll-down-indicator');
        setTimeout(function() {
            document.documentElement.classList.remove('scroll-enter');
            if (scrollEnterIndicator) scrollEnterIndicator.style.animation = 'none';
        }, 600);
    }
    ...
    // works.html: intercept forward navigation shortcuts to play exit animation
    var worksScrollIndicator = document.querySelector('.scroll-down-indicator');
    if (worksScrollIndicator && worksPageWrapper) {
        function playExitAndGoForward(e) {
            if (document.body.classList.contains('works-navigating-away')) return;
            e.preventDefault();
            worksPageWrapper.classList.add('fade-to-asterism');
            document.body.classList.add('works-navigating-away');
            setTimeout(function() { history.forward(); }, 580);
        }
        document.addEventListener('keydown', function(e) {
            if ((e.altKey && e.key === 'ArrowRight') || (e.metaKey && e.key === 'ArrowRight') || e.key === 'BrowserForward') {
                playExitAndGoForward(e);
            }
        });
        document.addEventListener('mouseup', function(e) {
            if (e.button === 4) { playExitAndGoForward(e); }
        });
    }
    ...
    // Back-to-top button on all works-related pages
    (function() {
        document.querySelectorAll('.music-gallery').forEach(function(gallery) {
            var wrap = document.createElement('div');
            wrap.className = 'back-to-top-wrap';
            var btn = document.createElement('button');
            btn.className = 'back-to-top';
            btn.setAttribute('aria-label', 'Back to top');
            btn.innerHTML = '<img src="https://file.garden/aX25ruuJQk0h7-cB/UpArrow" alt="" style="width:4.5rem;height:4.5rem;display:block;">';
            ...
            if ('IntersectionObserver' in window) {
                new IntersectionObserver(function(entries) {
                    entries.forEach(function(e) {
                        ...
                        btn.style.pointerEvents = e.isIntersecting ? 'auto' : 'none';
                    });
                }, { threshold: 1.0, rootMargin: '0px 0px -60px 0px' }).observe(wrap);
            }
        });
    })();
});
...
window.addEventListener('pageshow', function(e) {
    ...
    if (scrollIndicator) {
        ...
        // Always remove class first so re-adding it (or the reflow trick) reliably restarts the animation
        document.documentElement.classList.remove('scroll-enter');
    ...
    var worksMainNav = document.querySelector('.works-main-nav');
    if (worksMainNav) {
        document.documentElement.classList.remove('works-nav-animate');
        worksMainNav.offsetHeight;
        document.documentElement.classList.add('works-nav-animate');
        setTimeout(function() { document.documentElement.classList.remove('works-nav-animate'); }, 700);
    }
});
