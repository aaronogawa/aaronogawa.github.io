document.addEventListener('DOMContentLoaded', function() {
    ...
    var worksCategoryByTitle = { 'Music': 'music', 'Portraiture': 'portraiture', 'Design': 'design', 'Automotive': 'automotive', 'Highlights': 'highlights', 'Sports': 'sports', 'Event': 'events' };
            ...
            function renderSuggestions() {
                ...
                if (hasQuery && list.length === 0) {
                    ...
                    if (categoriesResults.length > 0 && remainingResults > 0) {
                        ...
                        var catsSlice = categoriesResults.slice(0, remainingResults);
                    ...
                    if (worksResults.length > 0 && remainingResults > 0) {
                        ...
                        var worksSlice = worksResults.slice(0, remainingResults);
                    ...
                    if (pageResults.length > 0 && remainingResults > 0) {
                        ...
                        var pagesSlice = pageResults.slice(0, remainingResults);
    ...
    function updateWorksHoverBar(rect, hoveredBtn) {
        ...
        var slug = href.indexOf('#') === 0 ? href.slice(1).toLowerCase() : '';
        // Works.html links are hash-based (e.g. "#music", "#automotive").
        // Pin the blue bar's left edge to either "Design" or "Automotive" so it stays consistent.
        var useDesignLeft = (slug === 'music' || slug === 'design' || slug === 'sports' || slug === 'events' || href === 'works/index.html');
        var anchorLink = useDesignLeft
            ? document.querySelector('.works-category-buttons .works-category-btn[href="#design"]')
            : document.querySelector('.works-category-buttons .works-category-btn[href="#automotive"]');
    ...
    function hideWorksPanels() {
        [worksDesignPanel, worksHighlightsPanel, worksMusicPanel, worksPortraiturePanel, worksAutomotivePanel, worksSportsPanel, worksEventsPanel, worksIllustrationsPanel].forEach(function(panel) {
            if (!panel) return;
    ...
    function showWorksPanelByHash(hash) {
        ...
        var panel = worksHashToPanel[slug] || worksHighlightsPanel;
        hideWorksPanels();
        if (panel) {
            panel.classList.add('is-visible');
            panel.setAttribute('aria-hidden', 'false');
    ...
    // Asterism / Bigfish pages: fade out when clicking the single "Back" button
    var galleryBackLink = document.querySelector('a.works-category-btn[href="../works.html#music"]');
    if (galleryBackLink) {
        ...
        function fadeOutThenNavigate(e) {
            if (e) e.preventDefault();
            if (e) e.stopPropagation();
            closeGalleryLightboxIfOpen();
            document.body.classList.add('gallery-page-fade-out');
            setTimeout(function() {
                window.location.href = galleryBackLink.href;
            }, 380);
        }
        galleryBackLink.addEventListener('click', fadeOutThenNavigate);
        galleryBackLink.addEventListener('keydown', function(e) {
            if (e.key !== 'Enter' && e.key !== ' ') return;
            e.preventDefault();
            fadeOutThenNavigate(e);
        });
    }
