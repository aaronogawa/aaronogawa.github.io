document.addEventListener('DOMContentLoaded', function() {
    ...
    var searchPages = [
        ...
        { title: 'Event', url: 'works.html#events', keywords: ['event', 'events', 'campus', 'coverage'], description: 'Event and campus coverage photography' },
    ...
    (function mergePageSearchKeywords() {
        ...
        var page = searchPages.filter(function(p) { return p.url === currentFile; })[0];
        if (page) {
            var seen = {};
    ...
    // On works.html, open a category panel by slug (music, portraiture, design, automotive) without navigating
    function openWorksCategoryPanelIfOnWorks(categorySlug) {
        var path = (window.location.pathname || '').split('/').pop() || '';
        if (path !== 'works.html') return false;
    ...
    var worksCategoryByTitle = { 'Music': 'music', 'Portraiture': 'portraiture', 'Design': 'design', 'Automotive': 'automotive', 'Sports': 'sports', 'Event': 'events' };
    function setupSearchSuggestions() {
        document.querySelectorAll('.nav-search-expanded').forEach(function(expanded) {
            ...
            function goToFirstMatch() {
                ...
                if (list.length > 0) window.location.href = list[0].url;
            }
            ...
            function renderSuggestions() {
                ...
                if (hasQuery && list.length === 0) {
                    ...
                    heading.className = 'nav-search-suggestions-heading';
            ...
            function syncAllSuggestionPanels() {
                ...
                document.querySelectorAll('.nav-search-expanded').forEach(function(exp) {
                    ...
                    if (hasQ && list.length === 0) {
                        ...
                        head.className = 'nav-search-suggestions-heading';
    ...
    // Works page: blue bar from right edge; Music/Design/Sports/Event use 5px left of "Design", Portraiture/Automotive use 5px left of "Automotive"
    ...
    function updateWorksHoverBar(rect, hoveredBtn) {
        if (!worksHoverBar) {
            ...
            worksHoverBar.style.cssText = 'position:fixed;background:#1b7ad6;z-index:0;pointer-events:none;transition:top 0.25s ease,height 0.25s ease,left 0.2s ease,right 0.2s ease,opacity 0.2s ease;opacity:0;';
        ...
        var useDesignLeft = (href === 'music.html' || href === 'design.html' || href === '#' || href === 'works/index.html');
        var anchorLink = useDesignLeft
            ? document.querySelector('.works-category-buttons .works-category-btn[href="design.html"]')
            : document.querySelector('.works-category-buttons .works-category-btn[href="automotive.html"]');
        var leftPx = anchorLink ? (anchorLink.getBoundingClientRect().left - WORKS_BAR_LEFT_INSET) : (rect.left - WORKS_BAR_LEFT_INSET);
    ...
    var worksDesignBtn = document.querySelector('.works-category-buttons .works-category-btn[href="design.html"]');
    var worksMusicBtn = document.querySelector('.works-category-buttons .works-category-btn[href="music.html"]');
    var worksPortraitureBtn = document.querySelector('.works-category-buttons .works-category-btn[href="portraiture.html"]');
    var worksAutomotiveBtn = document.querySelector('.works-category-buttons .works-category-btn[href="automotive.html"]');
    var worksSportsBtn = document.querySelector('.works-category-buttons .works-category-btn[data-category="sports"]');
    var worksEventsBtn = document.querySelector('.works-category-buttons .works-category-btn[data-category="events"]');

    function hideWorksPanels() {
        [worksDesignPanel, worksMusicPanel, worksPortraiturePanel, worksAutomotivePanel, worksSportsPanel, worksEventsPanel].forEach(function(panel) {
            if (!panel) return;
    ...
    if (worksDesignPanel && worksDesignBtn) {
        worksDesignBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideWorksPanels();
            worksDesignPanel.classList.add('is-visible');
            worksDesignPanel.setAttribute('aria-hidden', 'false');
        });
    }

    if (worksMusicPanel && worksMusicBtn) {
        worksMusicBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideWorksPanels();
            worksMusicPanel.classList.add('is-visible');
            worksMusicPanel.setAttribute('aria-hidden', 'false');
        });
    }

    if (worksPortraiturePanel && worksPortraitureBtn) {
        worksPortraitureBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideWorksPanels();
            worksPortraiturePanel.classList.add('is-visible');
            worksPortraiturePanel.setAttribute('aria-hidden', 'false');
        });
    }

    if (worksAutomotivePanel && worksAutomotiveBtn) {
        worksAutomotiveBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideWorksPanels();
            worksAutomotivePanel.classList.add('is-visible');
            worksAutomotivePanel.setAttribute('aria-hidden', 'false');
        });
    }

    if (worksSportsPanel && worksSportsBtn) {
        worksSportsBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideWorksPanels();
            worksSportsPanel.classList.add('is-visible');
            worksSportsPanel.setAttribute('aria-hidden', 'false');
        });
    }

    if (worksEventsPanel && worksEventsBtn) {
        worksEventsBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideWorksPanels();
            worksEventsPanel.classList.add('is-visible');
            worksEventsPanel.setAttribute('aria-hidden', 'false');
        });
    }

    // Works page: open with a category pre-selected from hash (e.g. from nav search: works.html#music)
    (function applyWorksHash() {
        var hash = (window.location.hash || '').replace(/^#/, '').toLowerCase();
        var panel = hash === 'music' ? worksMusicPanel : hash === 'portraiture' ? worksPortraiturePanel : hash === 'design' ? worksDesignPanel : hash === 'automotive' ? worksAutomotivePanel : hash === 'sports' ? worksSportsPanel : hash === 'events' ? worksEventsPanel : null;
        if (panel) {
            hideWorksPanels();
            panel.classList.add('is-visible');
            panel.setAttribute('aria-hidden', 'false');
    ...
    if (worksLightbox) {
        ...
        document.addEventListener('click', function(e) {
            var cell = e.target.closest('.works-design-panel .music-photo, .works-music-panel .music-photo, .works-portraiture-panel .music-photo, .works-automotive-panel .music-photo, .works-sports-panel .music-photo, .works-events-panel .music-photo');
